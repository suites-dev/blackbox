...
async function cleanupAll(input: {
  readonly inputs: readonly Awaited<ReturnType<typeof fixture>>[];
  readonly handles: readonly SandboxHandle[];
}): Promise<void> {
  const stops = await Promise.allSettled(
    input.handles.map((handle) => handle.stop({ reason: 'completed' })),
  );
  const cleanups = await Promise.allSettled(input.inputs.map(cleanup));
  const failures = [...stops, ...cleanups].flatMap((result) =>
    result.status === 'rejected' ? [result.reason as unknown] : [],
  );
  if (failures.length > 0) {
    throw new AggregateError(failures, 'Owned progress-proof cleanup failed');
  }
}

function inspectEarlyEvents(input: {
  ...
  const observed = input.events
    .slice(0, acquired)
    .flatMap((event) => (event.kind === 'acquisition-observation' ? [event.observation] : []));
  expect(observed).toContainEqual(
    expect.objectContaining({
      kind: 'service-state',
      container: expect.objectContaining({ state: 'running', health: 'healthy' }),
    }),
  );
  expect(observed).toContainEqual(
    expect.objectContaining({
      kind: 'resource-discovered',
      resource: expect.objectContaining({ kind: 'network' }),
    }),
  );
  for (const event of observed) {
    if (event.kind === 'service-state') {
      expect(event.container.containerName).toBe(`${input.projectName}-api-1`);
    }
    if (event.kind === 'resource-discovered') {
      expect(event.resource.name).toMatch(new RegExp(`^${input.projectName}_`, 'u'));
    }
  }
}

describe.skipIf(process.env.BLACKBOX_SANDBOX_DOCKER_TEST !== '1')(
  'live Compose acquisition progress',
  () => {
    it('observes two isolated projects before their delayed ports are ready and leaves no owned resources', async () => {
      const inputs = await Promise.all([
        fixture({ delay: 4, timeout: 20_000 }),
        fixture({ delay: 4, timeout: 20_000 }),
      ]);
      ...
      try {
        ...
        expect(
          proofs.every((proof) => proof.status === 'fulfilled'),
          JSON.stringify(proofs),
        ).toBe(true);
        const ports = proofs.flatMap((proof) =>
          proof.status === 'fulfilled' ? [proof.value] : [],
        );
    ...
    it('stops observation when up fails and retains observed states before the failure', async () => {
      ...
      try {
        ...
        expect(
          events.some(
            (event) =>
              event.kind === 'acquisition-observation' &&
              event.observation.kind === 'service-state',
          ),
        ).toBe(true);
