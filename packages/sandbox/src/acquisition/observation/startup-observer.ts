...
export function observeComposeStartup(input: StartupObserverInput): {
  readonly stop: () => Promise<void>;
} {
  if (input.mode.kind === 'silent') {
  ...
  const emit = (event: ComposeAcquisitionObservation): void => {
    if (controller.signal.aborted || input.mode.kind === 'silent') {
      return;
    }
    try {
      input.mode.emit(event);
    } catch {
      /* A display failure cannot change acquisition truth. */
    }
  };
  const poll = async (): Promise<void> => {
    try {
      ...
      if (available === false) {
        emit({ kind: 'observation-status', status: 'available' });
      }
      available = true;
      publishChanges({ snapshot, seen, emit });
    } catch {
      if (available !== false) {
        emit({ kind: 'observation-status', status: 'unavailable' });
      }
      available = false;
    }
