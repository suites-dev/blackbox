...
it('incorporates late descendants while keeping an independent shared-state trace session-only', async () => {
  ...
  expect(report.document.observations).toMatchObject({
    kind: 'collector-session-found',
    fragmentCount: 2,
    traces: {
      activityCorrelated: [{ traceId, activityIds: ['activity-late'] }],
      sessionOnly: [
        {
          kind: 'available',
          traceId: independent,
          association: { kind: 'session-only' },
          spans: [{ operation: 'independent-consumer' }],
        },
      ],
    },
  });
  ...
  for (const representation of [json, html]) {
    ...
    expect(representation).toContain('independent-consumer');
  }
});
