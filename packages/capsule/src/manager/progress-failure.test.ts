...
it('stops an acquired sandbox when progress persistence fails before manager readiness', async () => {
  ...
  const fixture = await requestFixture(async ({ reason }) => { stopped.push(reason); await restore(); });
  await new Promise<void>((resolve) => fixture.manager.server.close(() => { resolve(); }));
  try {
    await runCapsuleManager(fixture, {
      catalog: { load: () => Promise.resolve(catalogFixture(fixture.projectDirectory)), resolve: ({ catalog, systemId }) => resolveCatalogEntry({ catalog, selection: { kind: 'explicit-entry', entryId: systemId } }) },
      sandbox: { projectName: () => 'test-compose', start: async (input) => {
        const path = capsuleProgressPath(fixture);
        const bytes = await readFile(path, 'utf8');
        restore = async () => { await rm(path, { recursive: true }); await writeFile(path, bytes); };
        await rm(path);
        await mkdir(path);
        if (input.progress.kind !== 'events') { throw new Error('Expected progress sink'); }
        input.progress.sink.emit({ kind: 'acquisition-observation', sandboxId: fixture.executionId, projectName: 'test-compose', at: 'now', observation: { kind: 'waiting', elapsedMs: 0 } });
    ...
    expect(await readCapsuleRecord(fixture)).toMatchObject({ state: 'start-failed', cleanup: { kind: 'complete' } });
...
it('drains early observations before recording a rejected Compose acquisition', async () => {
  ...
  try {
    await runCapsuleManager(fixture, {
      catalog: { load: () => Promise.resolve(catalogFixture(fixture.projectDirectory)), resolve: ({ catalog, systemId }) => resolveCatalogEntry({ catalog, selection: { kind: 'explicit-entry', entryId: systemId } }) },
      sandbox: { projectName: () => 'test-compose', start: (input) => {
        if (input.progress.kind !== 'events') { throw new Error('Expected progress sink'); }
        input.progress.sink.emit({ kind: 'acquisition-observation', sandboxId: fixture.executionId, projectName: 'test-compose', at: 'now', observation: { kind: 'resource-discovered', resource: { kind: 'network', name: 'owned-network' } } });
        return Promise.reject(new Error('Compose refused startup'));
      } },
      now: () => new Date(),
    });
    const progress = await readCapsuleProgress(fixture);
    expect(progress.at(-2)).toMatchObject({ kind: 'acquisition-observation', observation: { kind: 'resource-discovered' } });
    expect(progress.at(-1)).toMatchObject({ kind: 'capsule-start-failed', stage: 'acquisition', cause: { message: 'Compose refused startup' } });
  } finally { await fixture.close(); }
});
...
it('retains both failures in the session record when acquisition and the progress journal fail', async () => {
  ...
  await new Promise<void>((resolve) => fixture.manager.server.close(() => { resolve(); }));
  try {
    await expect(runCapsuleManager(fixture, {
      catalog: { load: () => Promise.resolve(catalogFixture(fixture.projectDirectory)), resolve: ({ catalog, systemId }) => resolveCatalogEntry({ catalog, selection: { kind: 'explicit-entry', entryId: systemId } }) },
      sandbox: { projectName: () => 'test-compose', start: async (input) => {
    ...
    expect(record.error!.message).toContain('Acquisition failed: Compose refused startup');
    expect(record.error!.message).toContain('progress persistence failed:');
  } finally { await fixture.close(); }
});
