...
async function fixture() {
  ...
  const bootstrap = { projectDirectory, sessionId: 'quiet-river-ada', executionId: 'execution', systemId: 'orders', environment: {} };
  await mkdir(capsuleSessionDirectory(bootstrap), { recursive: true });
  const entry = { participants: { postgres: { service: 'db' } } } as CatalogEntry;
  const bridge = sandboxProgressBridge({ bootstrap, entry });
  if (bridge.mode.kind !== 'events') { throw new Error('Must retain progress even without a terminal'); }
  return { bootstrap, bridge, sink: bridge.mode.sink, base: { sandboxId: 'execution', projectName: 'owned', at: 'now' } };
}
...
it('persists early states in order, including Compose dependencies with no catalog participant', async () => {
  ...
  try {
    for (const service of ['db', 'dependency']) {
      input.sink.emit({ ...input.base, kind: 'acquisition-observation', observation: { kind: 'service-state', container: {
        service, containerId: service, containerName: `owned-${service}`, state: 'running', health: 'starting', termination: { kind: 'none' },
      } } });
    }
    input.sink.emit({ ...input.base, kind: 'acquisition-observation', observation: { kind: 'waiting', elapsedMs: 5000 } });
    await input.bridge.flush();
    const events = await readCapsuleProgress(input.bootstrap);
    expect(events).toMatchObject([
      { kind: 'acquisition-observation', sequence: 1, stage: 'acquisition', observation: { participant: 'postgres', container: { health: 'starting' } } },
  ...
  } finally { await rm(input.bootstrap.projectDirectory, { recursive: true, force: true }); }
});
...
it('captures asynchronous durable-write errors and rejects flush without an unhandled rejection', async () => {
  ...
  try {
    ...
    input.sink.emit({ ...input.base, kind: 'acquisition-observation', observation: { kind: 'waiting', elapsedMs: 0 } });
    input.sink.emit({ ...input.base, kind: 'acquisition-observation', observation: { kind: 'waiting', elapsedMs: 5000 } });
