...
${capsuleObservationSummaryScript}
function overview(d){const section=n('section','section');section.id='overview';add(section,title('CURRENT RECORD','Session overview','Lifecycle, readiness, telemetry, and cleanup come from distinct retained fields.'));const grid=n('div','panel summary-grid');const lifecycle=summaryCard('Lifecycle');const tone=d.lifecycle.kind==='running'?'good':d.lifecycle.kind==='failed'?'bad':d.lifecycle.kind==='starting'?'warn':'neutral';add(lifecycle,badge(d.lifecycle.kind,tone),p('Retained state '+d.session.retainedState),p('Compose project '+(available(d.composeProject)?d.composeProject.value:'not available')));const readiness=summaryCard('Entrypoint & readiness');if(available(d.readiness))add(readiness,badge(d.readiness.value.status,'good'),p(d.readiness.value.url+' · '+d.readiness.value.durationMs+'ms'));else add(readiness,badge('Unavailable'),p('No application readiness result was retained.'));add(readiness,p('Entrypoint '+(available(d.entrypoint)?d.entrypoint.value.url:'not available')));const telemetry=summaryCard('Observations');observationSummary(telemetry,d.observations);const cleanup=summaryCard('Cleanup');if(d.cleanup.kind==='failed')add(cleanup,badge('failed','bad'),p(d.cleanup.error.name+': '+d.cleanup.error.message));else add(cleanup,badge(d.cleanup.kind,d.cleanup.kind==='complete'?'good':'neutral'));add(cleanup,p('Artifact path [redacted]'));add(grid,lifecycle,readiness,telemetry,cleanup);add(section,grid);return section}
