...
const event = { kind: 'acquisition-observation', sessionId: 'test', sequence: 1, stage: 'acquisition', at: 'now', observation: { kind: 'waiting', elapsedMs: 5000 } };
...
it('exports a schema and rejects unsupported versions, malformed observations, and wrong identities', () => {
  expect(capsuleProgressSchema).toMatchObject({ $id: expect.stringContaining('capsule-progress-v1') });
...
it('reads legacy arrays and writes the versioned artifact without changing earlier events', async () => {
  ...
  try {
    ...
    await appendCapsuleProgress({ ...input, event: { kind: 'acquisition-observation', sessionId: 'test', observation: { kind: 'observation-status', status: 'unavailable' } } });
    const retained = JSON.parse(await readFile(capsuleProgressPath(input), 'utf8'));
    expect(retained).toMatchObject({ schemaVersion: 1, kind: 'capsule-progress', events: [event, { sequence: 2 }] });
