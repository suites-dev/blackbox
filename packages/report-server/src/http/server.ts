...
function validateInput(input: StartReportServerInput): void {
  if (!Number.isInteger(input.port) || input.port < 0 || input.port > 65_535) { throw new Error('Port must be an integer from 0 through 65535.'); }
...
function rejectedRequest(input: { request: IncomingMessage }): HttpResult | null {
  ...
  if (request.headers.host !== authority || (request.headers.origin !== undefined && request.headers.origin !== `http://${authority}`)) {
    return jsonResult({ status: 403, document: { kind: 'report-failure', code: 'invalid-request', message: 'Only requests from this local report origin are allowed.' } });
  }
  if (request.method !== 'GET' && request.method !== 'HEAD') {
    return jsonResult({ status: 405, document: { kind: 'report-failure', code: 'invalid-request', message: 'Reports are read-only; use GET or HEAD.' } });
  }
  return null;
}
...
async function handleRequest(input: { request: IncomingMessage; response: ServerResponse; config: StartReportServerInput }): Promise<void> {
  ...
  if (result === null) {
    ...
    } catch { result = failureResult({ failure: providerFailure() }); }
  }
...
function closeServer(input: { server: Server }): () => Promise<void> {
  ...
  return () => {
    ...
    closed = new Promise<void>((resolve, reject) => {
      const deadline = setTimeout(() => { input.server.closeAllConnections(); }, 1000);
      deadline.unref();
      input.server.close(error => { clearTimeout(deadline); if (error) { reject(error); } else { resolve(); } });
...
export async function startReportServer(input: StartReportServerInput): Promise<ReportServer> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    server.listen(input.port, '127.0.0.1', () => { server.off('error', reject); resolve(); });
  });
  const address = server.address();
  if (address === null || typeof address === 'string') { throw new Error('Report server did not acquire a TCP port.'); }
  return { kind: 'report-server', hostname: '127.0.0.1', port: address.port,
    url: `http://127.0.0.1:${address.port}/${selectionQuery(input)}`, close: closeServer({ server }) };
}
