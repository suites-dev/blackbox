...
test('serves a registry without eagerly loading reports or artifacts', async () => {
  ...
  expect(result.reports.map((report: { id: string }) => report.id)).toEqual([
    'exact-one',
    'exact-two',
  ]);
...
test('loads exact reports, renders through provider, and loads artifacts on demand', async () => {
  ...
  expect(await json.json()).toEqual({
    ...
    document: { id: 'exact-two', title: 'Orders' },
  });
  ...
  const artifact = await fetch(
    `${server.url}api/reports/capsule/exact-one/artifacts/activities.json`,
  );
  expect(await artifact.json()).toEqual({
    kind: 'report-artifact',
    artifact: 'activities.json',
    document: [],
  });
  expect(calls).toEqual([
    'load:exact-two',
    'load:exact-one',
    'render',
    'artifact:exact-one:activities.json',
  ]);
});
...
test('returns explicit missing, unavailable, and provider-error outcomes', async () => {
  ...
  expect(
    (await fetch(`${server.url}api/reports/capsule/exact-one/artifacts/secrets.json`)).status,
  ).toBe(422);
...
test('rejects writes, hostile origins, and encoded traversal before provider calls', async () => {
  ...
  expect(
    (await fetch(`${server.url}api/reports`, { headers: { origin: 'https://attacker.example' } }))
      .status,
  ).toBe(403);
  expect(await hostileHost({ url: `${server.url}api/reports` })).toBe(403);
  for (const path of [
    'api/reports/capsule/%2e%2e%2fsecret',
    'api/reports/capsule/%5csecret',
    'api/reports/capsule/%00',
    'api/reports/capsule/%FF',
  ]) {
...
test('rejects duplicate provider types and invalid initial selection before binding', async () => {
  ...
  await expect(startReportServer({ ...input, providers: [provider], port: -1 })).rejects.toThrow(
    'Port',
  );
  await expect(
    startReportServer({
      ...input,
      providers: [provider],
      selection: { kind: 'report', type: 'missing', id: 'one' },
    }),
