...
<main id="registry-content"><section class="hero"><p class="eyebrow">CAPSULE · EXPERIMENT REGISTRY</p><h1>Experiments</h1><p>Browse live and retained Capsule sessions. Reports follow read-only artifacts and refresh as their records change.</p><div class="stats" aria-label="Registry summary"><span><strong id="total-count">—</strong>experiments</span><span><strong id="running-count">—</strong>running</span><span><strong id="stopped-count">—</strong>stopped</span><span><strong id="failed-count">—</strong>failed</span></div></section>
<section class="browser" aria-label="Experiment browser"><aside><div class="aside-head"><div><p class="eyebrow">DISCOVERY</p><h2>Sessions</h2></div><p id="status" role="status" aria-atomic="true">Connecting…</p></div><label class="search"><span>Search experiments</span><input id="search" type="search" placeholder="Title, session ID, or type" autocomplete="off"></label><div class="controls"><div role="group" aria-label="Filter by session state"><button type="button" data-filter="all" aria-pressed="true">All</button><button type="button" data-filter="running" aria-pressed="false">Running</button><button type="button" data-filter="stopped" aria-pressed="false">Stopped</button><button type="button" data-filter="failed" aria-pressed="false">Failed</button></div><label>Sort <select id="sort"><option value="newest">Newest</option><option value="oldest">Oldest</option><option value="title">Title</option></select></label></div><p id="result-count" class="result-count"></p><ul id="reports"></ul></aside>
<section class="detail" aria-label="Selected experiment"><div id="detail-status" class="detail-status" role="status" aria-atomic="true"></div><div id="empty" class="empty"><p class="eyebrow">EXACT SESSION NAVIGATION</p><h2>Select an exact session</h2><p>The operational report will open here. Viewing a report never runs a command, mutates evidence, or changes a Capsule lifecycle.</p></div><div id="report" hidden></div></section></section></main>
