...
export function fixtureProvider() {
  ...
  const provider = {
    ...
    type: 'capsule',
    view: {
      kind: 'report-client-view',
      styles: '.fixture{}',
      script: `BlackboxReportViews.capsule={render(root,document){root.textContent=document.title}};`,
    },
    list() {
      calls.push('list');
    ...
    load(input) {
      ...
      if (input.id !== 'exact-one' && input.id !== 'exact-two') {
        return Promise.resolve({
          kind: 'report-failure',
          code: 'not-found',
          message: 'No such exact session.',
        } as const);
      }
      return Promise.resolve({
        kind: 'report-document',
        document: { id: input.id, title: 'Orders' },
      } as const);
    },
    artifact(input) {
    ...
    render() {
      calls.push('render');
      return '<!doctype html><h1>Shared renderer</h1>';
    },
