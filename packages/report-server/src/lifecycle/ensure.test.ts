...
afterEach(async () => { await Promise.all(close.splice(0).map(stop => stop())); });

function input(): EnsureReportServerInput {
  return { kind: 'ensure-report-server', scopeId: 'project-a', port: 0,
    selection: { kind: 'registry' }, providers: [fixtureProvider().provider] };
}

async function owned(config: EnsureReportServerInput) {
  const result = await ensureReportServer(config);
  if (result.kind !== 'report-server-started') { throw new Error('Expected an owned listener.'); }
...
test('reuses the same project listener with exact selection and grants no shutdown capability', async () => {
  ...
  const result = await ensureReportServer({ ...config, port: server.port,
    selection: { kind: 'report', type: 'capsule', id: 'exact-two' } });
  expect(result).toEqual({ kind: 'report-server-reused', hostname: '127.0.0.1', port: server.port,
  ...
  expect(metadata).toEqual({ kind: 'report-server-identity', schemaVersion: 1, scopeId: config.scopeId, providerTypes: ['capsule'] });
...
test('concurrent callers elect exactly one listener owner', async () => {
  ...
  const results: EnsureReportServerResult[] = await Promise.all(Array.from({ length: 6 }, async () => {
    const result = await ensureReportServer({ ...config, port });
    if (result.kind === 'report-server-started') { close.push(() => result.server.close()); }
    return result;
  }));
...
test('a different project or provider set cannot reuse or stop an existing viewer', async () => {
  const config = input();
  const server = await owned(config);
  await expect(ensureReportServer({ ...config, port: server.port, scopeId: 'project-b' })).rejects.toThrow('occupied');
  await expect(ensureReportServer({ ...config, port: server.port, providers: [] })).rejects.toThrow('occupied');
  expect((await fetch(server.url)).status).toBe(200);
  await expect(ensureReportServer({ ...config, port: server.port,
    selection: { kind: 'report', type: 'unknown', id: 'id' } })).rejects.toThrow('Initial report selection');
});

async function foreignServer(response: { kind: 'body'; value: string } | { kind: 'hung' }) {
  const server = createServer((_request, outgoing) => {
    if (response.kind === 'body') { outgoing.end(response.value); }
  });
  ...
  if (address === null || typeof address === 'string') { throw new Error('No foreign test port.'); }
  return { port: address.port, server };
}

test.each([
  'not JSON', 'null', '{}',
  JSON.stringify({ kind: 'report-server-identity', schemaVersion: 2, scopeId: 'project-a', providerTypes: ['capsule'] }),
  JSON.stringify({ kind: 'report-server-identity', schemaVersion: 1, scopeId: 'project-a', providerTypes: 'capsule' }),
  JSON.stringify({ kind: 'report-server-identity', schemaVersion: 1, scopeId: 'project-a', providerTypes: ['other'] }),
])('does not reuse an unrecognized listener: %s', async body => {
  const { port, server } = await foreignServer({ kind: 'body', value: body });
  await expect(ensureReportServer({ ...input(), port })).rejects.toThrow(`Report port ${port} is occupied`);
...
test('rejects invalid scope and bind inputs instead of treating them as reusable', async () => {
  await expect(ensureReportServer({ ...input(), scopeId: '/private/project' })).rejects.toThrow('opaque identifier');
