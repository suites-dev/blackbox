...
export function createCapsuleProgressRenderer(input: CapsuleProgressRendererInput): CapsuleProgressRenderer {
  if (input.presentation === 'silent') { return { sink: () => undefined, finish: () => undefined }; }
  ...
  const renderer = new InteractiveProgressRenderer({
    color: input.color, write: input.write, now: Date.now,
...
function isAcquisitionDetail(event: CapsuleProgressEvent): boolean {
  return ['acquisition-observation', 'container-acquired', 'endpoint-mapped', 'resource-owned'].includes(event.kind);
}
