...
function reportFailure(input: {
  ...
  if (input.result.kind === 'capsule-not-found') {
    return {
      kind: 'report-failure',
      code: 'not-found',
      message: 'The exact Capsule session does not exist.',
    };
  }
  return {
    kind: 'report-failure',
    code: 'artifact-unavailable',
    message: 'The retained Capsule report could not be read.',
  };
}
...
function summary(input: { entry: CapsuleRegistryEntry }): ReportSummary {
  if (input.entry.kind === 'capsule-session-summary') {
    ...
    return {
      ...
      title: item.title,
      description:
        item.description.kind === 'omitted'
          ? { kind: 'unavailable' }
          : { kind: 'available', value: item.description.value },
      state: item.state,
  ...
  return {
    kind: 'report-summary',
    type: 'capsule',
    id: input.entry.directoryName.replace(/^capsule-/u, ''),
    title: 'Unreadable Capsule session',
    description: { kind: 'unavailable' },
    state: input.entry.failure.kind,
...
function renderDocument(input: { kind: 'render-report'; document: unknown }): string {
  if (
    ...
    input.document === null ||
    !('kind' in input.document) ||
    input.document.kind !== 'capsule-operational-report'
  ) {
  ...
  return {
    ...
    async list() {
      ...
      if (result.kind === 'capsule-registry-failed') {
        return {
          kind: 'report-failure',
          code: 'artifact-unavailable',
          message: 'The Capsule session registry could not be read.',
        };
      }
    ...
    async load(request) {
      ...
      return result.kind === 'capsule-report'
        ? { kind: 'report-document', document: result.document }
    ...
    async artifact(request) {
      if (request.artifact !== 'report.json') {
        return {
          kind: 'report-failure',
          code: 'not-found',
          message: 'Only the redacted report.json projection is available for this provider.',
        };
      }
      const result = await reportCapsule({
        projectDirectory: input.projectDirectory,
