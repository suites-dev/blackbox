...
void test('browser launch preserves the exact URL as an argument without a shell', () => {
  ...
  for (const [platform, executable, args] of [
    ...
    assert.deepEqual(browserCommand({ kind: 'open-browser', platform, url }), {
      kind: 'command',
      executable,
      args,
    });
  }
  assert.deepEqual(browserCommand({ kind: 'open-browser', platform: 'unknown', url }), {
    kind: 'unsupported',
    platform: 'unknown',
  });
  for (const invalid of ['https://example.com/', 'file:///etc/passwd', 'http://localhost/']) {
    assert.throws(
      () => browserCommand({ kind: 'open-browser', platform: 'darwin', url: invalid }),
      /local report URL/u,
    );
  }
});
