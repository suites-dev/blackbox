...
void test('served registry selects an exact session, shares static renderer, and shuts down', async () => {
  ...
  try {
    ...
    const capsuleReport = await reportCapsule({ projectDirectory: fixture.directory, sessionId: fixture.sessionId });
    ...
    const exported = await runCli({ directory: fixture.directory, argv: ['capsule', 'report', 'export', '--session', fixture.sessionId, '--format', 'html', '--output', htmlPath] });
...
void test('report flags require an explicit nonconflicting mode and exact static selection', async () => {
  ...
  try {
    for (const args of [
      ['export'], ['export', '--session', fixture.sessionId],
      ...
      ['serve', '--format', 'html'], ['serve', '--output', 'out.html'], ['serve', '--port', '-1'],
      ['--serve'], ['--html'], ['--json'],
    ]) {
      const result = await runCli({ directory: fixture.directory, argv: ['capsule', 'report', ...args] });
