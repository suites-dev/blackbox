...
void test('replaces rows, erases removed rows, and never clears unrelated terminal content', () => {
  ...
  const block = new TerminalBlock({
    ...
    viewport: () => ({ columns: 70, rows: 24 }),
  });
  block.render({
    lines: ['header', 'old resource', 'old failure'].map((text) => ({ text, tone: 'neutral' })),
  });
  block.render({
    lines: [
      { text: 'new header', tone: 'active' },
      { text: 'new resource', tone: 'success' },
    ],
  });
...
void test('reflows the old frame after a width shrink and redraws only its own physical rows', () => {
  ...
  block.render({
    lines: [
      { text: 'h'.repeat(61), tone: 'active' },
      { text: 'r'.repeat(59), tone: 'neutral' },
    ],
  });
  columns = 23;
...
void test('keeps a header on very short screens and advertises omitted rows on bounded screens', () => {
  for (const rows of [2, 5]) {
    ...
    const block = new TerminalBlock({
      ...
      viewport: () => ({ columns: 45, rows }),
    });
    block.render({
      lines: Array.from({ length: 10 }, (_, index) => ({
        text: index === 0 ? 'Acquiring capsule' : `resource ${index}`,
    ...
    if (rows > 2) {
      assert.match(screen.text(), /more steps/u);
    }
  }
});
