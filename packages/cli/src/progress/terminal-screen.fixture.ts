...
export class TerminalScreen {
  ...
  writes = 0;
  wraps = 0;
  constructor(columns: number) { this.columns = columns; }
  ...
  write(text: string): void {
    ...
    for (const token of text.matchAll(pattern)) {
      ...
      if (token[0].startsWith(escape)) { this.escape({ count: Number(token[1] || 1), command: token[2] }); }
      else if (token[0] === '\r') { this.column = 0; }
      else if (token[0] === '\n') { this.row++; this.ensureLine(); }
      else { this.print(token[0]); }
    }
  ...
  resize(columns: number): void {
    ...
    for (const [index, line] of old.entries()) {
      ...
      for (let start = 0; start < chunks; start++) { this.lines.push(line.slice(start * columns, (start + 1) * columns)); }
    }
  }

  private escape(input: { readonly count: number; readonly command: string }): void {
    if (input.command === 'm') { return; }
    if (input.command === 'K') { assert.equal(input.count, 2); this.lines[this.row] = []; return; }
    if (input.command === 'A') { this.row -= input.count; assert.ok(this.row >= 1, 'renderer moved into unrelated output'); return; }
  ...
  private print(text: string): void {
    for (const { segment } of new Intl.Segmenter(undefined, { granularity: 'grapheme' }).segment(text)) {
      ...
      if (this.column + width > this.columns) { this.row++; this.column = 0; this.wraps++; }
      this.ensureLine();
      this.lines[this.row][this.column] = segment;
      for (let index = 1; index < width; index++) { this.lines[this.row][this.column + index] = ''; }
      this.column += width;
    }
  }
  private ensureLine(): void { this.lines[this.row] ??= []; }
}
