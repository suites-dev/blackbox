...
import type { CapsuleProgressEvent, CapsuleAcquisitionObservation } from '@suites/blackbox-capsule-internal';
import { InteractiveProgressRenderer } from './interactive-renderer.js';
import { TerminalScreen } from './terminal-screen.fixture.js';

const base = { sessionId: 'quiet-river-ada', sequence: 1, at: 'now', stage: 'acquisition' } as const;
const service = {
  kind: 'service-state', participant: 'postgres', container: { service: 'db', containerId: 'id', containerName: 'owned-db', state: 'created', health: 'starting', termination: { kind: 'none' } },
} satisfies CapsuleAcquisitionObservation;
function fixture() {
  const screen = new TerminalScreen(100);
  const renderer = new InteractiveProgressRenderer({ color: false, write: (text) => { screen.write(text); }, viewport: () => ({ columns: 100, rows: 24 }), now: Date.now });
  return { screen, renderer, observe: (observation: CapsuleAcquisitionObservation) => { renderer.sink({ ...base, kind: 'acquisition-observation', observation }); } };
}
...
void test('keeps spinner above stable rows and replaces service/resource states in place', (context) => {
  ...
  try {
    ...
    renderer.sink({ ...base, kind: 'resource-owned', resource: { kind: 'network', name: 'owned-net' } });
...
void test('keeps independent application readiness pending after a healthy Docker observation', () => {
  const { screen, renderer, observe } = fixture();
  observe({ ...service, container: { ...service.container, state: 'running', health: 'healthy' } });
  renderer.sink({ ...base, kind: 'container-acquired', participant: 'postgres', service: 'db', containerId: 'id', containerName: 'owned-db', networkNames: [] });
  renderer.sink({ ...base, stage: 'readiness', kind: 'readiness-started', url: 'http://localhost/health', timeoutMs: 5000 });
  ...
  renderer.sink({ ...base, stage: 'readiness', kind: 'readiness-succeeded', url: 'http://localhost/health', durationMs: 50 });
...
void test('keeps observation outages/recovery visible and failure final without a false stage success', (context) => {
  ...
  context.mock.timers.tick(5000); renderer.finish(); renderer.sink({ ...base, kind: 'capsule-ready', durationMs: 1 });
...
void test('all setup events occupy a single updating block and waiting uses retained elapsed time', () => {
  ...
  const events = [
    { ...base, kind: 'session-admitted', system: 'orders', artifactRoot: '/private', environmentKeys: ['TOKEN'] },
    { ...base, kind: 'manager-spawned', managerPid: 42 },
    { ...base, kind: 'catalog-selected', system: 'orders', configFile: '/private' },
    { ...base, kind: 'catalog-resolved', system: 'orders', projectDirectory: '/private', composeFiles: [], services: ['db'] },
    ...
    { ...base, kind: 'acquisition-observation', observation: { kind: 'waiting', elapsedMs: 10_000 } },
    { ...base, kind: 'endpoint-mapped', endpoint: { url: 'http://localhost', host: 'localhost', port: 80, protocol: 'http' } },
...
void test('a service failure changes its row without reordering neighbouring resources', () => {
  ...
  try {
    observe(service);
    observe({ ...service, participant: 'redis', container: { ...service.container, service: 'redis' } });
    const before = screen.text().split('\n').findIndex((line) => line.includes('redis —'));
    observe({ ...service, participant: 'redis', container: { ...service.container, service: 'redis', state: 'running', health: 'unhealthy' } });
    const after = screen.text().split('\n').findIndex((line) => line.includes('redis —'));
