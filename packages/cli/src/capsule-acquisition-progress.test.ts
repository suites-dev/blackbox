...
const observation = {
  kind: 'service-state', participant: 'postgres', container: { service: 'db', containerId: 'internal-id', containerName: 'internal-db', state: 'running', health: 'starting', termination: { kind: 'none' } },
...
void test('plain progress reports exit code, observation outages and recovery without control codes', () => {
  ...
  const observations = [
    { ...observation, container: { ...observation.container, state: 'exited', termination: { kind: 'exited', exitCode: 7 } } },
    { kind: 'observation-status', status: 'unavailable' }, { kind: 'observation-status', status: 'available' },
...
void test('interactive substeps mark observed unhealthy and terminated containers as failures', () => {
  ...
  try {
    for (const state of ['created', 'running', 'dead'] as const) {
      renderer.sink({ ...base, kind: 'acquisition-observation', observation: { ...observation, container: {
        ...observation.container, state, health: state === 'running' ? 'unhealthy' : 'not-configured',
