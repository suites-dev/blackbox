...
for (const mode of ['--silent', '--non-interactive', '--interactive']) {
  void test(`start ${mode} retains a failed manager session while keeping JSON stdout parseable`, async () => {
    ...
    try {
      ...
      if (mode === '--interactive') {
        assert.ok(
          result.stderr.includes('\u001b[2K'),
          'interactive redraw must use cursor control without color',
        );
      ...
      assert.deepEqual(
        (await readdir(sessionsRoot)).sort(),
        [fixture.sessionId, sessionId].map((id) => `capsule-${id}`).sort(),
      );
      const record = JSON.parse(
        await readFile(join(sessionsRoot, `capsule-${sessionId}`, 'session.json'), 'utf8'),
      );
      assert.equal(record.state, 'manager-failed');
      assert.equal(record.cleanup.kind, 'not-attempted');
      assert.equal(record.failure.kind, 'recorded');
      assert.ok(record.failure.error.message.length > 0);
      if (mode === '--silent') {
        assert.doesNotMatch(result.stderr, /session .* admitted|manager spawned/u);
      } else if (mode === '--interactive') {
        assert.match(result.stderr, /Session — /u);
      } else {
        assert.match(result.stderr, /session .* admitted/u);
      }
    } finally {
