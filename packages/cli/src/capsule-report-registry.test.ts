...
void test('served registry discovers newly retained sessions without changing exact selection', async () => {
  ...
  const child = await runningReportCli({ directory: fixture.directory, argv: ['--session', fixture.sessionId] });
  try {
    const origin = new URL(child.url).origin;
    const first = await (await fetch(`${origin}/api/reports`)).json();
    assert.deepEqual(first.reports.map((entry: { id: string }) => entry.id), [fixture.sessionId]);
    assert.equal(first.reports[0].title, 'Orders demo');
    assert.deepEqual(first.reports[0].description, { kind: 'available', value: 'Recorded orders experiment' });
    const sessionId = 'bright-meadow-zoe';
    const artifactRoot = capsuleSessionDirectory({ projectDirectory: fixture.directory, sessionId });
    await mkdir(artifactRoot);
    await writeFile(join(artifactRoot, 'session.json'), JSON.stringify({ ...fixture.record, sessionId, artifactRoot, system: 'payments' }));
    const refreshed = await (await fetch(`${origin}/api/reports/capsule`)).json();
    assert.deepEqual(refreshed.reports.map((entry: { id: string }) => entry.id).sort(), [sessionId, fixture.sessionId].sort());
  ...
  } finally { await child.stop(); await removeFixture(fixture.directory); }
});
...
void test('served artifact exports match JSON reports and deny raw retained files', async () => {
  ...
  try {
    ...
    const snapshot = await runCli({ directory: fixture.directory, argv: ['capsule', 'report', 'export', '--session', fixture.sessionId, '--format', 'json', '--output', '-'] });
    ...
    const missing = await fetch(`${origin}/api/reports/capsule/no-such-session/artifacts/report.json`);
    assert.equal(missing.status, 404);
  } finally { await child.stop(); await removeFixture(fixture.directory); }
});
...
void test('SIGTERM closes the CLI report listener and releases its port', async () => {
  ...
  try {
    ...
    const rebound = await runningReportCli({ directory: fixture.directory, argv: ['--port', new URL(child.url).port] });
    try { assert.equal((await fetch(rebound.url)).status, 200); }
    finally { assert.equal(await rebound.stop(), 0); }
  } finally { await child.stop(); await removeFixture(fixture.directory); }
});
...
void test('registry corruption stays visible and registry I/O failure cannot become empty success', async () => {
  ...
  try {
    ...
    assert.deepEqual(corrupt.reports.map((entry: { id: string; state: string }) => ({ id: entry.id, state: entry.state })), [
