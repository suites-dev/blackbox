...
export default class CatalogList extends Command {
  ...
  public async run(): Promise<void> { const { flags } = await this.parse(CatalogList); const output = renderCatalogOutput({ mode: flags.json ? 'json' : 'human', result: await runCatalogList({ projectDirectory: process.cwd() }) }); if (output.failed) {this.error(output.text, { exit: 1 });} this.log(output.text); }
}
